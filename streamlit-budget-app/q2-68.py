...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    c1.metric("พ.ร.บ. รวม (ลบ.)",           f"{total_all_prb:,.2f}")
    c2.metric("พ.ร.บ. ลงทุน (ลบ.)",         f"{total_inv_prb:,.2f}")
    c3.metric("งบฯ หลังโอนรวม (ลบ.)",       f"{total_all_after:,.2f}")
    c4.metric("งบฯ หลังโอน (ลงทุน) (ลบ.)",  f"{total_inv_after:,.2f}")
    c5.metric("% ลงทุนต่อ พ.ร.บ.",          f"{pct_inv_of_prb:.2f}%")
    c6.metric("% ลงทุนต่องบฯ หลังโอน",      f"{pct_inv_of_after:.2f}%")

    # แสดงคำอธิบายแบบกล่อง
    st.markdown(
        f"""
        <div class='summary-box'>
        ในปีงบประมาณ พ.ศ. 2568 มีการจัดสรรงบประมาณรวมทั้งสิ้น 
        <strong>{total_all_prb:,.2f} ล้านบาท</strong> โดยเป็นงบประมาณรายจ่ายลงทุน 
        <strong>{total_inv_prb:,.2f} ล้านบาท</strong> คิดเป็น <strong>{pct_inv_of_prb:.2f}%</strong> 
