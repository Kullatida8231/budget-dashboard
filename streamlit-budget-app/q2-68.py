...
#-----*********************************************************
#-----*********************************************************
# 🔧 เตรียมตารางสรุปตามกระทรวง
def prepare_ministry_table(df_filtered, label):
    df_grp = df_filtered.groupby("กระทรวง")[["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]].sum().reset_index()
    df_grp["%เบิกจ่าย"] = df_grp.apply(lambda row: (row["เบิกจ่าย"] / row["งบฯ หลังโอน"]) * 100 if row["งบฯ หลังโอน"] else 0, axis=1)
    df_grp["%ใช้จ่าย"] = df_grp.apply(lambda row: (row["ใช้จ่าย"] / row["งบฯ หลังโอน"]) * 100 if row["งบฯ หลังโอน"] else 0, axis=1)
    df_grp = df_grp[["กระทรวง", "พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "%เบิกจ่าย", "ใช้จ่าย", "%ใช้จ่าย"]]
    return df_grp

# 🔧 ฟังก์ชันไฮไลต์สีตามกลุ่ม
def highlight_ministry_group(row, group):
    try:
        color_disb = get_color(row["%เบิกจ่าย"], group, "เบิกจ่าย")
        color_spend = get_color(row["%ใช้จ่าย"], group, "ใช้จ่าย")
    except:
        color_disb = color_spend = "black"
    return ["", "", "", "", f"color: {color_disb}", "", f"color: {color_spend}"]

# 🔧 แสดงตารางแยกตามกลุ่ม
def show_ministry_section(df_filtered, group_label, icon):
    st.markdown(f"#### {icon} {group_label}")
    df_table = prepare_ministry_table(df_filtered, group_label)
    styled = df_table.style.format({
        ...
        "%เบิกจ่าย": "{:,.2f}%",
        "%ใช้จ่าย": "{:,.2f}%"
    }).apply(lambda row: highlight_ministry_group(row, group_label), axis=1)
...
if "1️⃣ ภาพรวมทั้งประเทศ" in selected_menus:
    ...
    # 🏛️ ตารางสรุปผลราย "กระทรวง" แยก 3 ส่วน
    st.markdown("### 🏛️ สรุปตามกระทรวง (แยก 3 ส่วน)")

    # 🔹 ภาพรวม
    show_ministry_section(df, "ภาพรวม", "🔹")

    # 🏢 รายจ่ายประจำ
    show_ministry_section(df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"], "รายจ่ายประจำ", "🏢")

    # 🏗️ รายจ่ายลงทุน
    show_ministry_section(df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"], "รายจ่ายลงทุน", "🏗️")
