...
<div style='text-align: left; font-size: 18px; font-weight: bold; margin-bottom: 10px;'>
 🔹 ข้อมูลผลการเบิกจ่ายงบประมาณและใช้จ่าย 
</div>
""", unsafe_allow_html=True)
...
# 🔹 แสดงผล
st.markdown(f"""
**{selected_ministry}** ได้รับจัดสรรงบประมาณ จำนวน **{total_prb:,.4f} ล้านบาท**  มีงบประมาณหลังโอนเปลี่ยนแปลง จำนวน **{total_after:,.4f} ล้านบาท**  มีการเบิกจ่าย จำนวน **{total_disb:,.4f} ล้านบาท**  (คิดเป็น <span style="color:{color_disb}; font-weight:bold;">{percent_disb:.2f}%</span> ของงบฯ หลังโอน)  และมีการใช้จ่าย จำนวน **{total_spend:,.4f}** ล้านบาท  (คิดเป็น <span style="color:{color_spend}; font-weight:bold;">{percent_spend:.2f}%</span> ของงบฯ หลังโอน)
""", unsafe_allow_html=True)
...
if not df_reg.empty:
    st.markdown(f"""
 **รายจ่ายประจำ** ได้รับจัดสรรงบประมาณ **{prb_r:,.4f} ล้านบาท**  มีงบประมาณหลังโอนเปลี่ยนแปลง จำนวน **{after_r:,.4f} ล้านบาท** มีการเบิกจ่าย จำนวน **{disb_r:,.4f} ล้านบาท**  (<span style="color:{color_disb_r}; font-weight:bold;">{per_disb_r:.2f}%</span> ของงบฯ หลังโอน) และมีการใช้จ่าย จำนวน **{spend_r:,.4f} ล้านบาท**  (<span style="color:{color_spend_r}; font-weight:bold;">{per_spend_r:.2f}%</span> ของงบฯ หลังโอน)
""", unsafe_allow_html=True)

if not df_inv.empty:
    st.markdown(f"""
 **รายจ่ายลงทุน** ได้รับจัดสรรงบประมาณ จำนวน **{prb_i:,.4f} ล้านบาท** มีงบประมาณหลังโอนเปลี่ยนแปลง จำนวน **{after_i:,.4f} ล้านบาท** มีการเบิกจ่าย จำนวน **{disb_i:,.4f} ล้านบาท**  (<span style="color:{color_disb_i}; font-weight:bold;">{per_disb_i:.2f}%</span> ของงบฯ หลังโอน) และมีการใช้จ่าย **{spend_i:,.4f} ล้านบาท**  (<span style="color:{color_spend_i}; font-weight:bold;">{per_spend_i:.2f}%</span> ของงบฯ หลังโอน)
""", unsafe_allow_html=True)
...
def highlight_central(row, disb_thres, spend_thres):
    ...
    return [
        "", f"", f"", f"", f"color: {color_disb}", f"", f"color: {color_spend}"
    ]
...
def show_central_table(df_subset, title, disb_thres, spend_thres):
    ...
    # 🔸 คำนวณ %เบิกจ่าย และ %ใช้จ่าย แบบใหม่
...
st.markdown("โดยสามารถจำแนกงบประมาณรายจ่ายออกเป็น 2 ประเภท ดังนี้", unsafe_allow_html=True)

if not df_plan_reg.empty:
    st.markdown(f"""
**รายจ่ายประจำ** ได้รับจัดสรรงบประมาณ **{prb_r:,.4f} ล้านบาท**  มีงบประมาณหลังโอนเปลี่ยนแปลง จำนวน **{after_r:,.4f} ล้านบาท**  เบิกจ่าย **{disb_r:,.4f} ล้านบาท**  (คิดเป็น <span style="color:{color_disb_r}; font-weight:bold;">{per_disb_r:.2f}%</span> ของงบฯ หลังโอน)  ใช้จ่าย **{spend_r:,.4f} ล้านบาท**  (คิดเป็น <span style="color:{color_spend_r}; font-weight:bold;">{per_spend_r:.2f}%</span> ของงบฯ หลังโอน)
""", unsafe_allow_html=True)

if not df_plan_inv.empty:
    st.markdown(f"""
**รายจ่ายลงทุน** ได้รับจัดสรรงบประมาณ **{prb_i:,.4f} ล้านบาท**  มีงบประมาณหลังโอนเปลี่ยนแปลง จำนวน **{after_i:,.4f} ล้านบาท**  เบิกจ่าย **{disb_i:,.4f} ล้านบาท**  (คิดเป็น <span style="color:{color_disb_i}; font-weight:bold;">{per_disb_i:.2f}%</span> ของงบฯ หลังโอน)  ใช้จ่าย **{spend_i:,.4f} ล้านบาท**  (คิดเป็น <span style="color:{color_spend_i}; font-weight:bold;">{per_spend_i:.2f}%</span> ของงบฯ หลังโอน)
""", unsafe_allow_html=True)
...
search_text = st.text_input("🔍 พิมพ์ชื่อบางส่วน", "", key="search_project_section7")
...
selected_dimension = st.selectbox("เลือกด้าน/ลักษณะงาน", sorted(df["ด้าน_ลักษณะงาน"].dropna().unique()))
