...
""", unsafe_allow_html=True)
    ...
    st.markdown("<br>", unsafe_allow_html=True)
#--------------------------------------------------------------
# 🔍 โค้ดตรวจสอบค่าที่แสดงผลผิดจาก threshold ที่ตั้งไว้
def check_color_logic(df_check, disb_thres, spend_thres, label="ไม่ระบุ"):
    df_check = df_check.copy()
    df_check["%เบิกจ่าย"] = round((df_check["เบิกจ่าย"] / df_check["งบฯ หลังโอน"]) * 100, 2)
    df_check["%ใช้จ่าย"] = round((df_check["ใช้จ่าย"] / df_check["งบฯ หลังโอน"]) * 100, 2)

    # ตรวจหาค่าที่ควรเป็นสีเขียวแต่ต่ำกว่า threshold
    wrong_disb = df_check[df_check["%เบิกจ่าย"] >= disb_thres]
    wrong_spend = df_check[df_check["%ใช้จ่าย"] >= spend_thres]

    st.markdown(f"### 🔍 ตรวจสอบ logic สีในส่วน: {label}")

    if wrong_disb.empty and wrong_spend.empty:
        st.success("✅ ไม่มีค่าผิดพลาดจากเกณฑ์ threshold")
    else:
        if not wrong_disb.empty:
            st.warning("⚠️ รายการที่ `%เบิกจ่าย` มากกว่าหรือเท่ากับ threshold แต่คุณสงสัยว่าแสดงเป็นสีแดง:")
            st.dataframe(wrong_disb[["หน่วยงาน", "%เบิกจ่าย", "เบิกจ่าย", "งบฯ หลังโอน"]], use_container_width=True)

        if not wrong_spend.empty:
            st.warning("⚠️ รายการที่ `%ใช้จ่าย` มากกว่าหรือเท่ากับ threshold แต่คุณสงสัยว่าแสดงเป็นสีแดง:")
            st.dataframe(wrong_spend[["หน่วยงาน", "%ใช้จ่าย", "ใช้จ่าย", "งบฯ หลังโอน"]], use_container_width=True)
#--------------------------------------------------------------
def prepare_table(df):
    df = df.copy()
    df["%เบิกจ่าย"] = round((df["เบิกจ่าย"] / df["งบฯ หลังโอน"]) * 100, 2)
    df["%ใช้จ่าย"] = round((df["ใช้จ่าย"] / df["งบฯ หลังโอน"]) * 100, 2)

    # ฟังก์ชันกำหนดสีในตาราง
    def colorize(row):
        type_ = row.get("รายจ่ายประจำ/ลงทุน", "")
        disb_thres = 53
        spend_thres = 61
        if type_ == "รายจ่ายประจำ":
            disb_thres = 57
            spend_thres = 58
        elif type_ == "รายจ่ายลงทุน":
            disb_thres = 35
            spend_thres = 66
