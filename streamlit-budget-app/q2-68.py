...
1. ข้อมูลผลการเบิกจ่ายงบประมาณและการใช้จ่ายของรัฐ จากระบบ New GFMIS Thai กรมบัญชีกลาง | แสดงข้อมูล ณ ตั้งแต่ต้นปี งปม. ถึงสิ้นเดือนสิงหาคม ปี งปม. 2568 | เรียกข้อมูล ณ วันที่ 8 กันยายน 2568
</div>
""", unsafe_allow_html=True)
st.markdown("""
<div style='text-align: left; font-size: 14px; font-weight: normal; margin-bottom: 10px;'>
...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    # 🔵 ติดตามผลการเบิกจ่ายและใช้จ่ายงบประมาณรายจ่ายลงทุน
st.markdown("## 🔵 ติดตามผลการเบิกจ่ายและใช้จ่ายงบประมาณรายจ่ายลงทุน")

# ---------- Safe styling (ใช้กล่องเดียวกับด้านบน ถ้ายังไม่ได้ประกาศให้วาง CSS block นี้ครั้งเดียวพอ) ----------
st.markdown("""
<style>
@media (prefers-color-scheme: light) {
  .summary-box { background:#f9f9f9; color:#000; }
}
@media (prefers-color-scheme: dark) {
  .summary-box { background:#1E1E1E; color:#fff; }
}
.summary-box { padding:15px; border-radius:10px; margin-bottom:12px; }
.kpi { font-size:14px; opacity:.85; margin-top:2px; }
</style>
""", unsafe_allow_html=True)

# ---------- ดึงยอดรวมจาก summary (กันหารศูนย์และกัน None) ----------
total_all_prb   = float(total_all.get("พรบ.", 0) or 0.0)            # งบ พ.ร.บ. รวมทั้งสิ้น
total_inv_prb   = float(total_invest.get("พรบ.", 0) or 0.0)         # งบ พ.ร.บ. (หมวดลงทุน)
total_all_after = float(total_all.get("งบฯ หลังโอน", 0) or 0.0)    # งบหลังโอนรวมทั้งสิ้น
total_inv_after = float(total_invest.get("งบฯ หลังโอน", 0) or 0.0) # งบหลังโอน (หมวดลงทุน)

# ---------- คำนวณสัดส่วน (%) ----------
pct_inv_of_prb   = round((total_inv_prb   / total_all_prb)   * 100, 2) if total_all_prb   else 0.00
pct_inv_of_after = round((total_inv_after / total_all_after) * 100, 2) if total_all_after else 0.00

# ---------- KPI แถวบน ----------
c1, c2, c3, c4, c5, c6 = st.columns(6)
c1.metric("พ.ร.บ. รวม (ลบ.)",        f"{total_all_prb:,.4f}")
c2.metric("พ.ร.บ. ลงทุน (ลบ.)",      f"{total_inv_prb:,.4f}")
c3.metric("งบฯ หลังโอนรวม (ลบ.)",    f"{total_all_after:,.4f}")
c4.metric("งบฯ หลังโอน (ลงทุน) (ลบ.)", f"{total_inv_after:,.4f}")
c5.metric("% ลงทุนต่อ พ.ร.บ.",       f"{pct_inv_of_prb:.2f}%")
c6.metric("% ลงทุนต่องบฯ หลังโอน",   f"{pct_inv_of_after:.2f}%")

# ---------- คำอธิบายแบบย่อ ในกล่อง ----------
st.markdown(
    f"""
<div class="summary-box">
ในปีงบประมาณ พ.ศ. 2568 มีการจัดสรรงบประมาณรวมทั้งสิ้น 
<strong>{total_all_prb:,.4f} ล้านบาท</strong> โดยเป็นงบประมาณรายจ่ายลงทุน 
<strong>{total_inv_prb:,.4f} ล้านบาท</strong> คิดเป็น <strong>{pct_inv_of_prb:.2f}%</strong> 
ของงบประมาณรายจ่ายรวมทั้งสิ้น และคิดเป็น <strong>{pct_inv_of_after:.2f}%</strong> 
ของงบประมาณหลังโอนเปลี่ยนแปลงทั้งสิ้น
</div>
""",
    unsafe_allow_html=True,
)
