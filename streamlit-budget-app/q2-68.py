...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    # ----- รวมยอดงบประมาณ (เปลี่ยนชื่อไม่ให้ชนกับ total_all ด้านบน) -----
    prb_all_sum   = float(df["พรบ."].sum() or 0.0)   # งบ พ.ร.บ. รวมทั้งสิ้น
    after_all_sum = float(df["งบฯ หลังโอน"].sum() or 0.0)  # งบฯ หลังโอนรวมทั้งสิ้น
    prb_inv_sum   = float(df.loc[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน", "พรบ."].sum() or 0.0)  # งบลงทุนรวม

    # ----- คำนวณสัดส่วน (%) -----
    ratio_invest_per_all   = round((prb_inv_sum / prb_all_sum) * 100, 2)   if prb_all_sum   else 0.0
    ratio_invest_per_trans = round((prb_inv_sum / after_all_sum) * 100, 2) if after_all_sum else 0.0
    ...
    st.markdown(
        f"""
        ในปีงบประมาณ พ.ศ. 2568 มีการจัดสรรงบประมาณรวมทั้งสิ้น **{prb_all_sum:,.2f} ล้านบาท** มีงบประมาณหลังโอนเปลี่ยนแปลงรวมทั้งสิ้น **{after_all_sum:,.2f} ล้านบาท**  
        มีงบประมาณรายจ่ายลงทุนรวมทั้งสิ้น **{prb_inv_sum:,.2f} ล้านบาท** คิดเป็นร้อยละ **{ratio_invest_per_all:.2f}%** ของงบประมาณรายจ่ายทั้งสิ้น และคิดเป็นร้อยละ **{ratio_invest_per_trans:.2f}%** ของงบประมาณรายจ่ายหลังโอนเปลี่ยนแปลงทั้งสิ้น
        """,
        unsafe_allow_html=True
    )
