...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("#### เบิกจ่าย (งบลงทุน)")
        st.metric("มูลค่า (ลบ.)", f"{total_invest_disb_disp:,.4f}")
        st.metric("% ต่อ พ.ร.บ.", f"{ratio_invest_disb_prb:.2f}%")
        st.metric("% ต่อ งบฯ หลังโอน", f"{ratio_invest_disb_trans:.2f}%")
    with c2:
        st.markdown("#### ใช้จ่าย (งบลงทุน)")
        st.metric("มูลค่า (ลบ.)", f"{total_invest_spend_disp:,.4f}")
        st.metric("% ต่อ พ.ร.บ.", f"{ratio_invest_spend_prb:.2f}%")
        st.metric("% ต่อ งบฯ หลังโอน", f"{ratio_invest_spend_trans:.2f}%")
