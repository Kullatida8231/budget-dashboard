...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    # ดึงยอดรวมจาก summary (กันหารศูนย์)
    total_all_prb   = float(total_all.get("พรบ.", 0) or 0)           # งบ พ.ร.บ. รวมทั้งสิ้น
    total_inv_prb   = float(total_invest.get("พรบ.", 0) or 0)        # งบ พ.ร.บ. (หมวดลงทุน)
    total_all_after = float(total_all.get("งบฯ หลังโอน", 0) or 0)   # งบหลังโอนรวมทั้งสิ้น
    total_inv_after = float(total_invest.get("งบฯ หลังโอน", 0) or 0)# งบหลังโอน (หมวดลงทุน)
    ...
    # แสดงคำอธิบาย
    st.markdown(
        f"""ในปีงบประมาณ พ.ศ. 2568 มีการจัดสรรงบประมาณรวมทั้งสิ้น 
        **{total_all_prb:,.4f} ล้านบาท** โดยเป็นงบประมาณรายจ่ายลงทุน **รวม {total_inv_prb:,.4f} ล้านบาท** 
        คิดเป็น **{pct_inv_of_prb:.2f}%** ของงบประมาณรายจ่ายรวมทั้งสิ้น และคิดเป็น 
        **{pct_inv_of_after:.2f}%** ของงบประมาณหลังโอนเปลี่ยนแปลงทั้งสิ้น""",
        unsafe_allow_html=True
    )
