...
st.markdown("""
    ...
    <div class='header-main'>ผลการเบิกจ่ายและใช้จ่ายงบประมาณ ณ สิ้นสุดเดือนสิงหาคม พ.ศ. 2568 (เทียบเป้าหมาย ณ สิ้นสุดเดือนสิงหาคม พ.ศ. 2568)</div>
""", unsafe_allow_html=True)
#-----------------------------
...
if "1️⃣ ภาพรวมทั้งประเทศ" in selected_menus:
    ...
    # ✅ ตาราง: รายกระทรวง (รวมภาพรวม, ประจำ, ลงทุน)
    st.markdown("### 🏛️ สรุปผลตามรายกระทรวง")
    tab1, tab2, tab3 = st.tabs(["📊 ภาพรวม", "🏢 รายจ่ายประจำ", "🏗️ รายจ่ายลงทุน"])

    with tab1:
        df_all_grouped = df.groupby("กระทรวง")[["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]].sum().reset_index()
        df_all_grouped["%เบิกจ่าย"] = (df_all_grouped["เบิกจ่าย"] / df_all_grouped["งบฯ หลังโอน"]) * 100
        df_all_grouped["%ใช้จ่าย"] = (df_all_grouped["ใช้จ่าย"] / df_all_grouped["งบฯ หลังโอน"]) * 100

        def highlight_all(row):
            styles = pd.Series("", index=row.index)
            styles["%เบิกจ่าย"] = f"color: {get_color(row['%เบิกจ่าย'], 'ภาพรวม', 'เบิกจ่าย')}"
            styles["%ใช้จ่าย"] = f"color: {get_color(row['%ใช้จ่าย'], 'ภาพรวม', 'ใช้จ่าย')}"
            return styles

        styled_all = df_all_grouped[["กระทรวง", "พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "%เบิกจ่าย", "ใช้จ่าย", "%ใช้จ่าย"]].style.format({
            "พรบ.": "{:,.4f}",
            "งบฯ หลังโอน": "{:,.4f}",
            "เบิกจ่าย": "{:,.4f}",
            "ใช้จ่าย": "{:,.4f}",
    ...
    with tab2:
        df_reg = df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"]
        df_reg_grouped = df_reg.groupby("กระทรวง")[["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]].sum().reset_index()
        df_reg_grouped["%เบิกจ่าย"] = (df_reg_grouped["เบิกจ่าย"] / df_reg_grouped["งบฯ หลังโอน"]) * 100
        df_reg_grouped["%ใช้จ่าย"] = (df_reg_grouped["ใช้จ่าย"] / df_reg_grouped["งบฯ หลังโอน"]) * 100

        def highlight_reg(row):
            styles = pd.Series("", index=row.index)
            styles["%เบิกจ่าย"] = f"color: {get_color(row['%เบิกจ่าย'], 'รายจ่ายประจำ', 'เบิกจ่าย')}"
            styles["%ใช้จ่าย"] = f"color: {get_color(row['%ใช้จ่าย'], 'รายจ่ายประจำ', 'ใช้จ่าย')}"
            return styles

        styled_reg = df_reg_grouped[["กระทรวง", "พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "%เบิกจ่าย", "ใช้จ่าย", "%ใช้จ่าย"]].style.format({
            "พรบ.": "{:,.4f}",
            "งบฯ หลังโอน": "{:,.4f}",
            "เบิกจ่าย": "{:,.4f}",
            "ใช้จ่าย": "{:,.4f}",
            "%เบิกจ่าย": "{:,.2f}%",
            "%ใช้จ่าย": "{:,.2f}%"
        }).apply(highlight_reg, axis=1)

        st.dataframe(styled_reg, use_container_width=True)

    with tab3:
        df_inv = df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"]
        df_inv_grouped = df_inv.groupby("กระทรวง")[["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]].sum().reset_index()
        df_inv_grouped["%เบิกจ่าย"] = (df_inv_grouped["เบิกจ่าย"] / df_inv_grouped["งบฯ หลังโอน"]) * 100
        df_inv_grouped["%ใช้จ่าย"] = (df_inv_grouped["ใช้จ่าย"] / df_inv_grouped["งบฯ หลังโอน"]) * 100

        def highlight_inv(row):
            styles = pd.Series("", index=row.index)
            styles["%เบิกจ่าย"] = f"color: {get_color(row['%เบิกจ่าย'], 'รายจ่ายลงทุน', 'เบิกจ่าย')}"
            styles["%ใช้จ่าย"] = f"color: {get_color(row['%ใช้จ่าย'], 'รายจ่ายลงทุน', 'ใช้จ่าย')}"
            return styles

        styled_inv = df_inv_grouped[["กระทรวง", "พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "%เบิกจ่าย", "ใช้จ่าย", "%ใช้จ่าย"]].style.format({
            "พรบ.": "{:,.4f}",
            "งบฯ หลังโอน": "{:,.4f}",
            "เบิกจ่าย": "{:,.4f}",
            "ใช้จ่าย": "{:,.4f}",
...
if "4️⃣ แผนบูรณาการ" in selected_menus:
    ...
    # แสดงตาราง
    show_plan_table(df_plan_selected, "ภาพรวม", disb_thres=87.67, spend_thres=93.33)
    if not df_plan_reg.empty:
        show_plan_table(df_plan_reg, "รายจ่ายประจำ", disb_thres=92, spend_thres=93.67)
    if not df_plan_inv.empty:
        show_plan_table(df_plan_inv, "รายจ่ายลงทุน", disb_thres=71.33, spend_thres=92.33)
...
if "5️⃣ จังหวัดและกลุ่มจังหวัด" in selected_menus:
    ...
    # 🔸 แสดงตารางภาพรวม
    show_local_table(df_selected, "ภาพรวม", disb_thres=87.67, spend_thres=93.33)

    # 🔸 Filter และแสดง รายจ่ายประจำ
    df_reg = df_selected[df_selected["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"]
    if not df_reg.empty:
        show_local_table(df_reg, "รายจ่ายประจำ", disb_thres=92, spend_thres=93.67)

    # 🔸 Filter และแสดง รายจ่ายลงทุน
    df_inv = df_selected[df_selected["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"]
    if not df_inv.empty:
        show_local_table(df_inv, "รายจ่ายลงทุน", disb_thres=71.33, spend_thres=92.33)
...
if "7️⃣ ผลผลิต/โครงการ (ค้นหาชื่อ)" in selected_menus:
    ...
    # ✅ ตาราง: ภาพรวม
    show_project_table(df_project, "ภาพรวม", disb_thres=87.67, spend_thres=93.33)

    # ✅ ตาราง: รายจ่ายประจำ
    df_reg = df_project[df_project["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"]
    if not df_reg.empty:
        show_project_table(df_reg, "จ่ายรายจ่ายประจำ", disb_thres=92, spend_thres=93.67)

    # ✅ ตาราง: รายจ่ายลงทุน
    df_inv = df_project[df_project["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"]
    if not df_inv.empty:
        show_project_table(df_inv, "จ่ายรายจ่ายลงทุน", disb_thres=71.33, spend_thres=92.33)
