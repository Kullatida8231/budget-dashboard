...
if "1️⃣ ภาพรวมทั้งประเทศ/กระทรวงหรือเทียบเท่ากระทรวง" in selected_menus:
    ...
    total_invest = float(df.loc[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน", "พรบ."].sum() or 0.0)  

    # รวมยอดเบิกจ่ายและใช้จ่าย เฉพาะหมวดลงทุน
    total_invest_disb  = float(df.loc[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน", "เบิกจ่าย"].sum() or 0.0)
    total_invest_spend = float(df.loc[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน", "ใช้จ่าย"].sum() or 0.0)
    ...
    st.markdown(
        ...
        มีการจัดสรรงบประมาณรวมทั้งสิ้น **{total_prb_disp:,.4f} ล้านบาท**  
    ...
    table_df = pd.DataFrame(
        [
            [total_invest_disb_disp,  total_invest_spend_disp],  # แถวที่ 1: มูลค่ารวม
            [ratio_invest_disb_prb,   ratio_invest_spend_prb],   # แถวที่ 2: % เทียบ พ.ร.บ.
            [ratio_invest_disb_trans, ratio_invest_spend_trans], # แถวที่ 3: % เทียบงบฯ หลังโอน
        ],
        columns=["เบิกจ่าย", "ใช้จ่าย"]
    )

    # ตั้งชื่อ index ของแต่ละแถว
    table_df.index = [
        "มูลค่ารวม (ลบ.)",
        "% เทียบ พ.ร.บ.",
        "% เทียบงบฯ หลังโอน"
    ...
    # ฟอร์แมตตัวเลข: แถวแรกเป็นทศนิยม 4 ตำแหน่ง, แถวที่ 2-3 เป็น %
    def _fmt_cell(val, row_idx):
        if row_idx == 0:
            return f"{val:,.4f}"
        else:
            return f"{val:.2f}%"
    ...
    styled_table = table_df.style.format({
        col: (lambda v, ri=i: _fmt_cell(v, ri)) for i, col in enumerate(table_df.columns)
    })
