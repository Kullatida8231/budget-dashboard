...
if "1️⃣ ภาพรวมทั้งประเทศ" in selected_menus:
    ...
    # 🏛️ ตารางสรุปผลราย "กระทรวง"
    st.markdown("### 🏛️ สรุปตามกระทรวง")

    # 🔸 สรุปตาราง
    df_ministry = df.groupby("กระทรวง")[["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]].sum().reset_index()

    # ป้องกันการหารศูนย์ และ NaN
    df_ministry["%เบิกจ่าย"] = df_ministry.apply(
        lambda row: (row["เบิกจ่าย"] / row["งบฯ หลังโอน"]) * 100 if row["งบฯ หลังโอน"] else 0,
        axis=1
    )
    df_ministry["%ใช้จ่าย"] = df_ministry.apply(
        lambda row: (row["ใช้จ่าย"] / row["งบฯ หลังโอน"]) * 100 if row["งบฯ หลังโอน"] else 0,
        axis=1
    )

    # เลือกเฉพาะคอลัมน์สำคัญ
    df_ministry = df_ministry[[
        "กระทรวง", "พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "%เบิกจ่าย", "ใช้จ่าย", "%ใช้จ่าย"
    ]]
    ...
    # 🔸 Apply Styler
    styled_ministry = df_ministry.style.format({
        "พรบ.": "{:,.4f}",
        "งบฯ หลังโอน": "{:,.4f}",
        "เบิกจ่าย": "{:,.4f}",
        "ใช้จ่าย": "{:,.4f}",
    ...
    st.dataframe(styled_ministry, use_container_width=True)
