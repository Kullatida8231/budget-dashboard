...
if "1️⃣ ภาพรวมทั้งประเทศ" in selected_menus:
    ...
    def summarize_ministry_text(df_subset, disb_thres, spend_thres):
        ...
        # หน่วยงานต่ำกว่าเป้า
        low_disb = df_grouped[df_grouped["%เบิกจ่าย"] < disb_thres]["หน่วยงาน"].tolist()
        low_spend = df_grouped[df_grouped["%ใช้จ่าย"] < spend_thres]["หน่วยงาน"].tolist()

        return len(df_grouped), low_disb, low_spend

    # 📊 ภาพรวม
    total_all, low_disb_all, low_spend_all = summarize_ministry_text(df, disb_thres=87.67, spend_thres=93.33)

    # 🏢 รายจ่ายประจำ
    df_regular = df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"]
    total_reg, low_disb_reg, low_spend_reg = summarize_ministry_text(df_regular, disb_thres=92, spend_thres=93.67)

    # 🏗️ รายจ่ายลงทุน
    df_invest = df[df["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"]
    total_inv, low_disb_inv, low_spend_inv = summarize_ministry_text(df_invest, disb_thres=71.33, spend_thres=92.33)

        # ======================== 🔵 สรุปผล กระทรวง/เทียบเท่ากระทรวง ======================== #
    def summarize_ministry_text(df_subset, disb_thres, spend_thres):
        # รวมข้อมูลราย "กระทรวง"
        group_cols = ["กระทรวง"]
        sum_cols = ["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]
        df_grouped = df_subset.groupby(group_cols, as_index=False)[sum_cols].sum(numeric_only=True)
    ...
    # ======================== 🔵 แสดงผลสรุปเป็นข้อความ ======================== #
    st.markdown(f"""
    📊 **ในภาพรวม** มีกระทรวงทั้งหมด **{total_all} กระทรวง** ที่มีผลการเบิกจ่ายต่ำกว่าเป้าหมาย 
    ได้แก่ {", ".join(low_disb_all) if low_disb_all else "-"}  
    และมีกระทรวงทั้งหมด **{len(low_spend_all)} กระทรวง** ที่มีผลการใช้จ่ายต่ำกว่าเป้าหมาย 
    ได้แก่ {", ".join(low_spend_all) if low_spend_all else "-"}  

    เมื่อจำแนกตามประเภทงบประมาณรายจ่ายพบว่า  

    • **🏢 รายจ่ายประจำ** มีกระทรวงทั้งหมด **{total_reg} กระทรวง** ที่มีผลการเบิกจ่ายต่ำกว่าเป้าหมาย 
      ได้แก่ {", ".join(low_disb_reg) if low_disb_reg else "-"}  
      และมีกระทรวงทั้งหมด **{len(low_spend_reg)} กระทรวง** ที่มีผลการใช้จ่ายต่ำกว่าเป้าหมาย 
      ได้แก่ {", ".join(low_spend_reg) if low_spend_reg else "-"}  

    • **🏗️ รายจ่ายลงทุน** มีกระทรวงทั้งหมด **{total_inv} กระทรวง** ที่มีผลการเบิกจ่ายต่ำกว่าเป้าหมาย 
      ได้แก่ {", ".join(low_disb_inv) if low_disb_inv else "-"}  
      และมีกระทรวงทั้งหมด **{len(low_spend_inv)} กระทรวง** ที่มีผลการใช้จ่ายต่ำกว่าเป้าหมาย 
      ได้แก่ {", ".join(low_spend_inv) if low_spend_inv else "-"}  

    ---
    ดังนั้น กระทรวงที่มีผลการเบิกจ่ายงบประมาณและมีผลการใช้จ่ายเงินของรัฐต่ำกว่าเป้าหมาย ควรเร่งรัดให้มีการเบิกจ่ายและใช้จ่ายให้เป็นไปตามค่าเป้าหมายตามมาตรการฯ เพื่อให้การใช้จ่ายงบประมาณเป็นไปตามวัตถุประสงค์ของการขอรับจัดสรรงบประมาณ ทั้งในด้านการพัฒนาประเทศ การแก้ไขปัญหาความเดือดร้อนให้กับประชาชน และการยกระดับความเป็นอยู่ของประชาชน
    """, unsafe_allow_html=True)
