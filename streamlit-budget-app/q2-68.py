...
if "4️⃣ แผนบูรณาการ" in selected_menus:
    ...
    # ================= ตารางสรุปตามชื่อแผนงานบูรณาการ (รวม/ประจำ/ลงทุน) ================= st.markdown("<hr>", unsafe_allow_html=True) st.markdown( """<div style='text-align: left; font-size: 18px; font-weight: bold; margin: 10px 0;'> 🔵 ตารางสรุปตามชื่อแผนงานบูรณาการ (เลือกดู: ภาพรวม / รายจ่ายประจำ / รายจ่ายลงทุน) </div>""", unsafe_allow_html=True ) # ---------- Helper ---------- import numpy as np import pandas as pd def _safe_pct(numer, denom): try: return round((numer / denom) * 100, 2) if denom and denom != 0 else 0.00 except Exception: return 0.00 def _collapse(df_subset, suffix): """สรุปยอดตามแผนงาน แล้วเติม suffix แยกชุดคอลัมน์""" sum_cols = ["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"] g = df_subset.groupby("แผนงาน", as_index=False)[sum_cols].sum(numeric_only=True) g[f"%เบิกจ่าย{suffix}"] = [_safe_pct(x, y) for x, y in zip(g["เบิกจ่าย"], g["งบฯ หลังโอน"])] g[f"%ใช้จ่าย{suffix}"] = [_safe_pct(x, y) for x, y in zip(g["ใช้จ่าย"], g["งบฯ หลังโอน"])] g = g.rename(columns={ "พรบ.": f"พรบ.{suffix}", "งบฯ หลังโอน": f"งบฯ หลังโอน{suffix}", "เบิกจ่าย": f"เบิกจ่าย{suffix}", "ใช้จ่าย": f"ใช้จ่าย{suffix}", }) g.replace([np.inf, -np.inf], 0, inplace=True) g.fillna(0, inplace=True) return g # ---------- Build three views ---------- df_plan_all = df_plan.copy() df_overall = _collapse(df_plan_all, " (รวม)") df_regonly = _collapse(df_plan_all[df_plan_all["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายประจำ"], " (ประจำ)") df_invonly = _collapse(df_plan_all[df_plan_all["รายจ่ายประจำ/ลงทุน"] == "รายจ่ายลงทุน"], " (ลงทุน)") # ---------- Select display columns ---------- cols_overall = [ "แผนงาน", "พรบ. (รวม)", "งบฯ หลังโอน (รวม)", "เบิกจ่าย (รวม)", "%เบิกจ่าย (รวม)", "ใช้จ่าย (รวม)", "%ใช้จ่าย (รวม)" ] cols_reg = [ "แผนงาน", "พรบ. (ประจำ)", "งบฯ หลังโอน (ประจำ)", "เบิกจ่าย (ประจำ)", "%เบิกจ่าย (ประจำ)", "ใช้จ่าย (ประจำ)", "%ใช้จ่าย (ประจำ)" ] cols_inv = [ "แผนงาน", "พรบ. (ลงทุน)", "งบฯ หลังโอน (ลงทุน)", "เบิกจ่าย (ลงทุน)", "%เบิกจ่าย (ลงทุน)", "ใช้จ่าย (ลงทุน)", "%ใช้จ่าย (ลงทุน)" ] view_overall = df_overall[[c for c in cols_overall if c in df_overall.columns]].copy() view_reg = df_regonly[[c for c in cols_reg if c in df_regonly.columns]].copy() view_inv = df_invonly[[c for c in cols_inv if c in df_invonly.columns]].copy() # ---------- Sort by total budget ---------- if "งบฯ หลังโอน (รวม)" in view_overall.columns: view_overall = view_overall.sort_values("งบฯ หลังโอน (รวม)", ascending=False) if "งบฯ หลังโอน (ประจำ)" in view_reg.columns: view_reg = view_reg.sort_values("งบฯ หลังโอน (ประจำ)", ascending=False) if "งบฯ หลังโอน (ลงทุน)" in view_inv.columns: view_inv = view_inv.sort_values("งบฯ หลังโอน (ลงทุน)", ascending=False) # ---------- Style & Threshold ---------- fmt_money = "{:,.4f}" fmt_pct = "{:,.2f}%" def _style_df(df_show, mode: str): """mode in {'overall','reg','inv'}""" if df_show.empty: return df_show.style fmt_dict = { col: fmt_money for col in df_show.columns if any(k in col for k in ["พรบ.", "งบฯ หลังโอน", "เบิกจ่าย", "ใช้จ่าย"]) } fmt_dict.update({ col: fmt_pct for col in df_show.columns if "%เบิกจ่าย" in col or "%ใช้จ่าย" in col }) def _row_style(row): styles = [] for col in df_show.columns: style = "" if mode == "overall": if col == "%เบิกจ่าย (รวม)": style = "color: #00FF9F;" if row[col] >= th_overall_disb else "color: #FF4B4B;" elif col == "%ใช้จ่าย (รวม)": style = "color: #00FF9F;" if row[col] >= th_spend else "color: #FF4B4B;" elif mode == "reg": if col == "%เบิกจ่าย (ประจำ)": style = "color: #00FF9F;" if row[col] >= th_reg_disb else "color: #FF4B4B;" elif col == "%ใช้จ่าย (ประจำ)": style = "color: #00FF9F;" if row[col] >= th_spend else "color: #FF4B4B;" elif mode == "inv": if col == "%เบิกจ่าย (ลงทุน)": style = "color: #00FF9F;" if row[col] >= th_inv_disb else "color: #FF4B4B;" elif col == "%ใช้จ่าย (ลงทุน)": style = "color: #00FF9F;" if row[col] >= th_spend else "color: #FF4B4B;" styles.append(style) return styles return df_show.style.format(fmt_dict).apply(_row_style, axis=1) # ---------- UI: ปุ่มเลือกตาราง ---------- if "plan_view" not in st.session_state: st.session_state.plan_view = "overall" # ค่าเริ่มต้น c1, c2, c3 = st.columns(3) with c1: if st.button("📊 แสดง: ภาพรวม", use_container_width=True): st.session_state.plan_view = "overall" with c2: if st.button("🏢 แสดง: รายจ่ายประจำ", use_container_width=True): st.session_state.plan_view = "reg" with c3: if st.button("🏗️ แสดง: รายจ่ายลงทุน", use_container_width=True): st.session_state.plan_view = "inv" # ---------- Render ตามปุ่มที่เลือก ---------- if st.session_state.plan_view == "overall": st.markdown("#### 📊 ตาราง: ภาพรวม") styled_table = _style_df(view_overall, mode="overall") st.dataframe(styled_table, use_container_width=True) st.download_button( "⬇️ ดาวน์โหลด CSV (ภาพรวม)", data=view_overall.to_csv(index=False).encode("utf-8-sig"), file_name="integration_plans_overall.csv", mime="text/csv" ) elif st.session_state.plan_view == "reg": st.markdown("#### 🏢 ตาราง: รายจ่ายประจำ") if view_reg.empty: st.info("ไม่มีข้อมูลรายจ่ายประจำ") else: styled_table = _style_df(view_reg, mode="reg") st.dataframe(styled_table, use_container_width=True) st.download_button( "⬇️ ดาวน์โหลด CSV (รายจ่ายประจำ)", data=view_reg.to_csv(index=False).encode("utf-8-sig"), file_name="integration_plans_recurrent.csv", mime="text/csv" ) elif st.session_state.plan_view == "inv": st.markdown("#### 🏗️ ตาราง: รายจ่ายลงทุน") if view_inv.empty: st.info("ไม่มีข้อมูลรายจ่ายลงทุน") else: styled_table = _style_df(view_inv, mode="inv") st.dataframe(styled_table, use_container_width=True) st.download_button( "⬇️ ดาวน์โหลด CSV (รายจ่ายลงทุน)", data=view_inv.to_csv(index=False).encode("utf-8-sig"), file_name="integration_plans_investment.csv", mime="text/csv" )
