...
if "2️⃣ กระทรวง/หน่วยงาน" in selected_menus:
    ...
    # 🔧 ฟังก์ชันกำหนดสีข้อความ
    def get_color(val, threshold):
        return "#00FF9F" if val >= threshold else "#FF4B4B"

    # 🔧 ฟังก์ชันจัดรูปแบบตารางพร้อมสีตาม threshold
    def prepare_table(df):
        ...
        def apply_color(row):
            spend_type = row.get("รายจ่ายประจำ/ลงทุน", "")
            if spend_type == "รายจ่ายประจำ":
                disb_thres = 57
                spend_thres = 58
            elif spend_type == "รายจ่ายลงทุน":
                disb_thres = 35
                spend_thres = 66
            else:
                disb_thres = 53
            ...
            color_disb = "color: #00FF9F" if row["%เบิกจ่าย"] >= disb_thres else "color: #FF4B4B"
            color_spend = "color: #00FF9F" if row["%ใช้จ่าย"] >= spend_thres else "color: #FF4B4B"
            return ["", "", "", "", color_disb, "", color_spend]
