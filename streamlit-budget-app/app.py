...
st.markdown("""
    ...
    '>
        ⚠️ อยู่ระหว่างการพัฒนาระบบ (Under development) ⚠️
    </div>
...
# --- ภาพรวม ---
st.markdown("<div class='sub-header'>🏛️ งบประมาณรายจ่ายภาพรวม</div>", unsafe_allow_html=True)
...
# --- รายจ่ายประจำ ---
st.markdown("<div class='sub-header'>📜 งบประมาณรายจ่ายประจำ</div>", unsafe_allow_html=True)
...
# --- รายจ่ายลงทุน ---
st.markdown("<div class='sub-header'>🏗️ งบประมาณรายจ่ายลงทุน</div>", unsafe_allow_html=True)
...
st.divider()
st.markdown("<div class='uniform-font'>2️⃣ กระทรวง</div>", unsafe_allow_html=True)
...
# --- รายจ่ายประจำ ---
st.markdown("<div class='sub-header'>📜 รายจ่ายประจำ</div>", unsafe_allow_html=True)
...
# --- รายจ่ายลงทุน ---
st.markdown("<div class='sub-header'>🏗️ รายจ่ายลงทุน</div>", unsafe_allow_html=True)
...
st.divider()
st.markdown("<div class='uniform-font'>3️⃣หน่วยงาน</div>", unsafe_allow_html=True)

# --- Dropdown เลือกกระทรวง ---
selected_ministry = st.selectbox("🔎เลือกกระทรวง", df1['กระทรวง'].dropna().unique())
...
# --- กลุ่มที่ 1: ภาพรวม ---
st.subheader("🏛️ งบประมาณรายจ่ายภาพรวม")
...
st.subheader("📜 งบประมาณรายจ่ายประจำ")
...
st.subheader("🏗️ งบประมาณจ่ายรายจ่ายลงทุน")
...
st.divider()
st.markdown("<div class='uniform-font'>4️⃣ ยุทธศาสตร์การจัดสรรงบประมาณ</div>", unsafe_allow_html=True)
...
st.markdown(f"<div class='sub-header'> <code>{selected_strategy}</code></div>", unsafe_allow_html=True)
...
st.divider()
st.markdown("<div class='uniform-font'>5️⃣ งบประมาณรายจ่ายจำแนกรายด้าน</div>", unsafe_allow_html=True)
...
# ชื่อด้านที่เลือก
st.markdown(f"<div class='sub-header'> <code>{selected_dimension}</code></div>", unsafe_allow_html=True)
...
st.divider()
st.markdown("<div class='uniform-font'>6️⃣ งบประมาณรายจ่ายงบกลาง</div>", unsafe_allow_html=True)
...
# --- กลุ่มที่ 3: รายจ่ายลงทุน ---
st.subheader("🏗️ งบประมาณรายจ่ายลงทุน")
df_invest = filtered_df_all[filtered_df_all['รายจ่ายประจำ/ลงทุน'] == "รายจ่ายลงทุน"]
df_grouped_invest = aggregate_table(df_invest, "รายจ่ายลงทุน")
styled_invest = (
    df_grouped_invest.drop(columns=['ประเภท'])
...
st.divider()
st.markdown("<div class='uniform-font'>7️⃣ งบประมาณจำแนกตามกลุ่มแผนงาน</div>", unsafe_allow_html=True)
...
st.divider()
st.markdown("<div class='uniform-font'>8️⃣ งบประมาณจำแนกตามจังหวัด/กลุ่มจังหวัด</div>", unsafe_allow_html=True)
...
st.dataframe(styled_invest, use_container_width=True)
#----------------------------------------------------------------------------------
# 🔹 ส่วนที่ 9: งบประมาณของหน่วยงานรัฐสภา
st.divider()
...
st.divider()
st.markdown("<div class='uniform-font'>🔟 ผลผลิต/โครงการ</div>", unsafe_allow_html=True)

# --- ช่องกรอกข้อความเพื่อกรองชื่อโครงการ ---
search_text = st.text_input("🔎พิมพ์ชื่อโครงการ")
...
# --- แสดงชื่อกระทรวงและหน่วยงานของโครงการ ---
if not filtered_df_all.empty:
    ministry = filtered_df_all['กระทรวง'].iloc[0] if 'กระทรวง' in filtered_df_all else "-"
    department = filtered_df_all['หน่วยงาน'].iloc[0] if 'หน่วยงาน' in filtered_df_all else "-"
    st.markdown(f"<div class='sub-header'>📌 โครงการนี้อยู่ภายใต้ <b>กระทรวง:</b> <code>{ministry}</code> | <b>หน่วยงาน:</b> <code>{department}</code></div>", unsafe_allow_html=True)
...
# --- ฟังก์ชันรวม Grand Total ---
def get_grand_total(df):
    prb = df['พรบ.(ล้านบาท)'].sum()
    after = df['งบฯ หลังโอน(ล้านบาท)'].sum()
    disb = df['เบิกจ่าย(ล้านบาท)'].sum()
    spend = df['ใช้จ่าย(ล้านบาท)'].sum()
    percent_disb = round((disb / after) * 100, 2) if after else 0
    percent_spend = round((spend / after) * 100, 2) if after else 0
    return prb, after, disb, spend, percent_disb, percent_spend

# --- ฟังก์ชันแสดง Grand Total ด้วยสี ---
def show_grand_total(prb, after, disb, spend, percent_disb, percent_spend, disb_thres, spend_thres):
    color_disb = "green" if percent_disb > disb_thres else "red"
    color_spend = "green" if percent_spend > spend_thres else "red"

    st.markdown(
        f"<div style='font-size:18px; margin-top:10px; margin-bottom:30px;'><b>รวมทั้งสิ้น</b> | "
        f"<b>พรบ.:</b> {prb:,.4f} | <b>หลังโอน:</b> {after:,.4f} | <b>เบิกจ่าย:</b> {disb:,.4f} | "
        f"<b style='color:{color_disb}'>%เบิกจ่าย: {percent_disb:.2f}%</b> | <b>ใช้จ่าย:</b> {spend:,.4f} | "
        f"<b style='color:{color_spend}'>%ใช้จ่าย: {percent_spend:.2f}%</b></div>",
        unsafe_allow_html=True
    )
...
show_grand_total(*get_grand_total(df_all), disb_thres=27, spend_thres=27)
...
show_grand_total(*get_grand_total(df_grouped_regular), disb_thres=35, spend_thres=36)
...
show_grand_total(*get_grand_total(df_grouped_invest), disb_thres=17, spend_thres=39)
...
st.markdown("""
    ...
    '>
        ...
        📧 Email: <a href="mailto:pbo@parliament.go.th">pbo@parliament.go.th</a><br>
        💬 LINE ID: <strong>@thaipbo</strong>
    </div>
""", unsafe_allow_html=True)
